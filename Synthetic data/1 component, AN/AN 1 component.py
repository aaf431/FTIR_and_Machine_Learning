...
def assign_noise(spectra_point, max_p, weight):
    #errors = np.concatenate((errors_vector,-errors_vector))
    #idx = np.random.randint(len(errors))
...
def test_model_noise(model,nf,pca_param,pca,range_opt):
    ...
    errors = (np.abs(y_predict-y_test.values.ravel())/y_test.values.ravel())*100
    errors_abs = (np.abs(y_predict-y_test.values.ravel()))
    #mean_err = np.mean(errors)
    mean_err = np.mean((np.abs(y_predict-y_test.values.ravel()))*100)
...
#Saving metrics in a dictionary
import pickle

f = open("dict_AN_1_MAE.pkl","wb")
